package review

import (
	"errors"
	"net/http"
	"net/http/httptest"
	"testing"
	"time"

	"github.com/m31-labs/rostrum/examples/demo/fixture"
	"github.com/m31-labs/rostrum/internal/appstate"
	"github.com/m31-labs/rostrum/internal/domain"
	"github.com/m31-labs/rostrum/internal/store"
	"github.com/m31-labs/rostrum/internal/token"
	"m31labs.dev/gosx/action"
)

// scoreMutationStore returns the preflight snapshot, then applies a separate
// valid state transition before scoreSubmission acquires its write lock. It
// models an organizer changing a plan, roster, or managed assignment in the
// small interval between a reviewer loading the form and saving it.
type scoreMutationStore struct {
	store.StateStore
	mutate  func(*domain.State) error
	mutated bool
}

func (store *scoreMutationStore) Snapshot() domain.State {
	snapshot := store.StateStore.Snapshot()
	if !store.mutated {
		store.mutated = true
		if err := store.StateStore.Update(store.mutate); err != nil {
			panic(err)
		}
	}
	return snapshot
}

func TestScoreSubmissionReauthorizesTheCurrentReviewState(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*domain.State)
	}{
		{name: "closed plan", mutate: func(state *domain.State) { state.ReviewPlans[1].Status = "closed" }},
		{name: "removed roster member", mutate: func(state *domain.State) {
			state.ReviewPlans[1].ReviewerIDs = []string{"rev_ada", "rev_marcus", "rev_virtual_practitioner"}
		}},
		{name: "removed managed assignment", mutate: func(state *domain.State) {
			state.ReviewAssignments[0].RemovedAt = time.Now().UTC()
			state.ReviewAssignments[0].RemovalReason = "reviewer reassigned"
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			now := time.Now().UTC()
			state := fixture.Seed(now)
			state.ReviewPlans[1].AssignmentsManaged = true
			state.ReviewAssignments = []domain.ReviewAssignment{{
				ID: "assign_current_state", PlanID: "plan_round_two", SubmissionID: "sub_workspace", ReviewerID: "rev_ines",
				Source: "manual", Actor: "organizer:test", AssignedAt: now,
			}}
			workspace, err := store.Open(":memory:", state)
			if err != nil {
				t.Fatalf("open workspace: %v", err)
			}
			appstate.Set(&scoreMutationStore{
				StateStore: workspace,
				mutate: func(current *domain.State) error {
					test.mutate(current)
					return nil
				},
			})

			reviewerToken := token.NewReviewer().SignReviewer("rev_ines")
			request := httptest.NewRequest(http.MethodPost, "/review/"+reviewerToken+"/__actions/scoreSubmission", nil)
			request.SetPathValue("token", reviewerToken)
			err = scoreSubmission(&action.Context{
				Request: request,
				FormData: map[string]string{
					"plan_id":              "plan_round_two",
					"submission_id":        "sub_workspace",
					"recommendation":       "yes",
					"comments":             "This review is valid until the current authorization state changes.",
					"score_program_fit":    "5",
					"score_evidence":       "4",
					"score_novelty":        "4",
					"score_audience_value": "5",
				},
			})
			var result *action.ResultError
			if !errors.As(err, &result) {
				t.Fatalf("scoreSubmission error = %v, want structured validation failure", err)
			}
			if _, found := reviewerEvaluation(workspace.Snapshot(), "plan_round_two", "sub_workspace", "rev_ines"); found {
				t.Fatal("score persisted after current-state authorization changed")
			}
		})
	}
}
